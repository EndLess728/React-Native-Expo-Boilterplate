...
├── app.config.ts             # Dynamic Expo config (name, package per env)
...
|---|---|---|---|
| `development` | ExpoTemplate (Dev) | `com.expo.template.dev` | `com.expo.template.dev` |
| `staging` | ExpoTemplate (Staging) | `com.expo.template.staging` | `com.expo.template.staging` |
| `production` | ExpoTemplate | `com.expo.template` | `com.expo.template` |

This means you can install **all three variants on the same device** simultaneously.
...
- **Package names change per environment** — committing one env's native code causes conflicts with others
...
1. **Update package names** in `app.config.ts` — change `com.expo.template` to your actual bundle identifier
